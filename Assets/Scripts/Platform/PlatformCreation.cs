...
public class PlatformCreation : MonoBehaviour
{
    ...
    private GameObject newParentCopy;
    ...
    private void Awake()
    {
        ...
        newParent = GameObject.Find("PlatformParent"); 
    ...
    {    
        ...
        if (onTop == true)
        {
            if(first == true)
            {
                newParentCopy = Instantiate(newParent, transform.position, transform.rotation);
                first = false;
                checkUpdate = false;
            }
            newParentCopy.transform.SetParent(transform);
             player.transform.SetParent(newParentCopy.transform);
        }
        else if (checkUpdate == false)
        {
            newParentCopy.transform.parent = null;
